module Refinery
  module Admin
    class CopywritingPhrasesController < ::Refinery::AdminController
      ...
      def find_all_copywriting_phrases
        @copywriting_phrases = Refinery::CopywritingPhrase.where(:page_id => nil).order([:scope, :name])
      ...
      def find_all_scopes
        @scopes ||= Refinery::CopywritingPhrase.select(:scope).where(:page_id => nil).map(&:scope).uniq
      end
